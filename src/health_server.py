...
def _list_meetings() -> list[dict]:
    ...
    try:
        rows = conn.execute(
            """
            SELECT meet_code, event_id, scheduled_start_utc, scheduled_end_utc, title, status,
                   organizer, attendees,
...
def _audio_segment_payloads(paths: list[Path]) -> list[dict]:
    ...
    for index, path in enumerate(paths):
        ...
        payload["peaks"] = _audio_peaks(path) if payload.get("exists") and payload["duration_seconds"] else []
...
function badge(status){const labels={delivered:'Done',failed:'Fail',scheduled:'Sched',joining:'Join',recording:'Rec',processing:'Proc',no_one_joined:'Empty',cancelled:'Cancel'}; return `<span class="status ${esc(status)}">${esc(labels[status]||status)}</span>`}
...
${kv('Status',statusHtml)}${kv('Meet code',meetCodeTools(m.meet_code))}${kv('Event ID',esc(m.event_id))}${kv('Host',esc(m.organizer||''))}${kv('Attendees',attendeeList(m.attendees))}${kv('Start',fmt(m.scheduled_start_utc))}${kv('End',fmt(m.scheduled_end_utc))}${kv('Delivered',fmt(m.delivered_at))}${kv('Listen',audioPlayer(m))}${m.last_error?kv('Error',esc(m.last_error)):''}
${codeBlock('Meeting Minutes',files.minutes?.content||'',{pdf:true})}${codeBlock('Summary',files.summary?.content||'')}${codeBlock('Transcript',files.transcript?.content||'')}${codeBlock('Notes',files.notes?.content||'')}</div>`; initAudioPlayers();}
