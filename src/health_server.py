...
def _request_regenerate(meet_code: str, payload: dict) -> dict:
    ...
    try:
        ...
        repo.mark_processing(meet_code, "queued", 0, 0)
...
${kv('Status',statusHtml)}${kv('AI processing',processingLine(m))}${kv('Meeting ended',meetingEndedLine(m))}${kv('Meet code',meetCodeTools(m.meet_code))}${kv('Event ID',esc(m.event_id))}${kv('Host',esc(m.organizer||''))}${kv('Attendees',attendeeList(m.attendees))}${kv('Start',fmt(m.scheduled_start_utc))}${kv('End',fmt(m.scheduled_end_utc))}${kv('Actual meet end',fmt(m.actual_end_utc))}${kv('Delivered',fmt(m.delivered_at))}${kv('Listen',audioPlayer(m))}${m.last_error?kv('Error',esc(m.last_error)):''}
${instructionBox(m)}${codeBlock('Meeting Minutes',files.minutes?.content||'',{pdf:true})}${codeBlock('Summary',files.summary?.content||'')}${codeBlock('Transcript',files.transcript?.content||'')}${codeBlock('Notes',files.notes?.content||'')}</div>`; initAudioPlayers();}catch(e){if(lock){detailLoadingCode=''; renderMeetings(currentPage,{push:false});} document.getElementById('detail').innerHTML=`<div class="empty-detail error">Failed to load meeting: ${esc(e.message||String(e))}</div>`; throw e;}}
...
async function regenerate(code){const instruction=document.getElementById('adminInstruction')?.value||''; const ok=await confirmDialog('Regenerate AI output?', 'Transcript, summary, minutes, and notes will be rebuilt from saved audio.', 'Regenerate'); if(!ok)return; pendingAction={code,fromStatus:''}; startDetailPolling(code); try{const result=await api('meetings/'+encodeURIComponent(code)+'/regenerate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({admin_instruction:instruction})}); if(result.error)throw new Error(result.error); await loadDetail(code,{push:false,lock:false}); await notify('success','Regeneration queued',code);}catch(e){pendingAction=null; stopDetailPolling(); await notify('error','Regeneration failed',e.message||String(e));}}
...
function instructionBox(m){return `<section class="instruction-box"><h3>Admin instruction</h3><textarea id="adminInstruction" placeholder="Add context for this meeting, speaker mapping, terms, or what to emphasize before regenerating...">${esc(m.admin_instruction||'')}</textarea><div class="instruction-actions"><button onclick="regenerate('${esc(m.meet_code)}')">Regenerate AI</button><span class="muted">Rebuild transcript, summary, minutes, and notes from saved audio.</span></div></section>`;}
