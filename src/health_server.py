...
.muted{color:#94a3b8}.status{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:2px 8px;background:#1e293b;color:#c7d2fe;font-size:12px;line-height:1.35}.status:before{content:"";width:7px;height:7px;border-radius:999px;background:currentColor}.status.failed{background:#451a1a;color:#fecaca}.status.delivered{background:#102f20;color:#86efac}.status.no_one_joined,.status.recorded{background:#1f2937;color:#cbd5e1}.status.recording,.status.processing-running{background:#422006;color:#fde68a}.status.processing-done{background:#102f20;color:#86efac}.status.processing-failed{background:#451a1a;color:#fecaca}.status.scheduled{background:#172554;color:#93c5fd}.status.joining{background:#312e81;color:#c4b5fd}.status.upcoming{background:#0c2d48;color:#7dd3fc}.status.checking{background:#172554;color:#bfdbfe}.status.checking .dots span,.status.processing-running .dots span{animation:dotPulse 1.2s infinite;opacity:.25}.status.checking .dots span:nth-child(2),.status.processing-running .dots span:nth-child(2){animation-delay:.2s}.status.checking .dots span:nth-child(3),.status.processing-running .dots span:nth-child(3){animation-delay:.4s}@keyframes dotPulse{0%,80%,100%{opacity:.25}40%{opacity:1}}
...
.audio-loader{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.audio-stage.is-hidden{display:none}.continuous-player{display:flex;flex-direction:column;gap:8px;max-width:680px}.audio-toolbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.audio-main{min-width:72px}.segments-row{display:flex;flex-wrap:wrap;gap:6px}.segment-chip{height:28px;padding:0 8px;border-radius:999px;font-size:12px;display:inline-flex;align-items:center;gap:5px}.segment-chip span{color:#94a3b8;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px}.segment-chip.active{background:#0c4a6e;border-color:#0284c7;color:#e0f2fe}.segment-chip.active span{color:#bae6fd}.segment-chip:disabled{opacity:.45;cursor:not-allowed}.progress-bar{height:34px;background:#111827;border-radius:8px;cursor:pointer;overflow:hidden;border:1px solid #334155;position:relative}.progress-bar.loading{cursor:wait}.waveform-bg{position:absolute;inset:3px 4px;display:flex;align-items:center;gap:1px;opacity:.7;pointer-events:none}.wave-bar{flex:1;min-width:1px;border-radius:999px;background:#475569;opacity:.22}.wave-bar.loaded{background:#7dd3fc;opacity:.65}.load-fill{position:absolute;left:0;top:0;bottom:0;width:0;background:rgba(148,163,184,.12);pointer-events:none}.progress-fill{position:absolute;left:0;top:0;bottom:0;width:0;background:linear-gradient(90deg,rgba(56,189,248,.28),rgba(56,189,248,.08));border-right:2px solid #38bdf8;pointer-events:none}.rate-btns{display:flex;gap:4px}.rate-btn{height:26px;padding:0 7px;font-size:12px}.rate-btn.active{background:#0c4a6e;border-color:#0284c7;color:#e0f2fe}.audio-note{font-size:12px}.login{min-height:100vh;display:grid;place-items:center;background:#070b12}.login-box{width:min(360px,calc(100vw - 32px));background:#0f172a;border:1px solid #263244;border-radius:8px;padding:20px;display:flex;flex-direction:column;gap:10px}.login-box input{height:36px;border:1px solid #334155;background:#070b12;color:#e5e7eb;border-radius:6px;padding:0 10px}.error{color:#fca5a5;margin:0}audio{width:min(560px,100%);height:34px}
...
function processingBadge(m){const state=String(m.processing_status||''); if(!state||state==='idle')return ''; const total=Number(m.processing_total||0); const batch=Number(m.processing_batch||0); if(state==='running')return `<span class="status processing-running">AI ${batch||0}/${total||'?'}<span class="dots"><span>.</span><span>.</span><span>.</span></span></span>`; if(state==='done')return `<span class="status processing-done">AI Done</span>`; if(state==='failed')return `<span class="status processing-failed">AI Fail</span>`; return `<span class="status">${esc(state)}</span>`;}
...
${kv('Status',statusHtml)}${kv('AI processing',processingLine(m))}${kv('Meet code',meetCodeTools(m.meet_code))}${kv('Event ID',esc(m.event_id))}${kv('Host',esc(m.organizer||''))}${kv('Attendees',attendeeList(m.attendees))}${kv('Start',fmt(m.scheduled_start_utc))}${kv('End',fmt(m.scheduled_end_utc))}${kv('Actual meet end',fmt(m.actual_end_utc))}${kv('Delivered',fmt(m.delivered_at))}${kv('Listen',audioPlayer(m))}${m.last_error?kv('Error',esc(m.last_error)):''}
${codeBlock('Meeting Minutes',files.minutes?.content||'',{pdf:true})}${codeBlock('Summary',files.summary?.content||'')}${codeBlock('Transcript',files.transcript?.content||'')}${codeBlock('Notes',files.notes?.content||'')}</div>`; initAudioPlayers();}catch(e){if(lock){detailLoadingCode=''; renderMeetings(currentPage,{push:false});} document.getElementById('detail').innerHTML=`<div class="empty-detail error">Failed to load meeting: ${esc(e.message||String(e))}</div>`; throw e;}}
...
function startDetailPolling(code){watchedMeeting=code; if(detailPollTimer)clearInterval(detailPollTimer); detailPollTimer=setInterval(async()=>{if(!watchedMeeting)return; await loadMeetings(); const row=allMeetings.find(m=>m.meet_code===watchedMeeting); if(row&&pendingAction?.code===watchedMeeting&&row.status!==pendingAction.fromStatus)pendingAction=null; await loadDetail(watchedMeeting,{push:false,lock:false}); if(row&&!isActiveStatus(row.status)&&!pendingAction)stopDetailPolling();},3000);}
