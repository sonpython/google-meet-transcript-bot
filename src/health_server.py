...
button,input,.button-link{height:32px;border:1px solid #334155;background:#182235;color:#e5e7eb;border-radius:6px;padding:0 10px}button{cursor:pointer}button:hover,.button-link:hover{background:#22304a;border-color:#475569}.button-link{display:inline-flex;align-items:center;text-decoration:none}input::placeholder{color:#64748b}.settings-panel{max-width:760px}.settings-row{display:grid;grid-template-columns:180px 120px auto 1fr;gap:10px;align-items:center;padding:12px 14px}.filters{display:grid;grid-template-columns:minmax(0,1fr) minmax(112px,126px) minmax(112px,126px) auto;gap:8px;padding:12px 14px;border-bottom:1px solid #263244}.filters input{min-width:0}.filters input[type=date]{padding:0 6px;font-size:13px}.row{padding:10px 14px;border-bottom:1px solid #1f2937;cursor:pointer}.row:hover{background:#152033}
...
table{width:100%;border-collapse:collapse}td,th{text-align:left;padding:8px 10px;border-bottom:1px solid #1f2937}.meet-mini{display:flex;align-items:center;gap:5px;color:#64748b;font-size:11px;line-height:1.2;margin-bottom:4px}.meet-mini span{overflow-wrap:anywhere}.mini-copy{width:22px;height:22px;padding:0;display:inline-flex;align-items:center;justify-content:center;background:transparent;border-color:transparent;color:#94a3b8}.mini-copy:hover{background:#1e293b;border-color:#334155}.mini-copy svg{width:13px;height:13px}.login{min-height:100vh;display:grid;place-items:center;background:#070b12}.login-box{width:min(360px,calc(100vw - 32px));background:#0f172a;border:1px solid #263244;border-radius:8px;padding:20px;display:flex;flex-direction:column;gap:10px}.login-box input{height:36px;border:1px solid #334155;background:#070b12;color:#e5e7eb;border-radius:6px;padding:0 10px}.error{color:#fca5a5;margin:0}audio{width:min(560px,100%);height:34px;filter:invert(1) hue-rotate(180deg)}
...
function meetMini(code){if(!code)return ''; return `<div class="meet-mini"><span>${esc(code)}</span><button class="mini-copy" onclick="copyText('${esc(code)}',this)" title="Copy meet ID"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button></div>`;}
...
async function loadMeetings(){const d=await api('meetings'); allMeetings=d.meetings; renderMeetings(currentPage);}
...
async function copyText(text,button){await navigator.clipboard.writeText(text); const old=button.innerHTML; button.textContent='OK'; setTimeout(()=>button.innerHTML=old,900);}
""" + boot + "\n"
